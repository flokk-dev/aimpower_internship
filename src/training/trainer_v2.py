...
from .pipeline import StablePipeline, GStablePipeline


class TrainerV2(Trainer):
    """
    Represents a TrainerV2.
    ...
    """
    _PIPELINES = {"basic": StablePipeline, "guided": GStablePipeline}
    ...
    def __init__(
        ...
        """
        # Mother class
        super(TrainerV2, self).__init__(params)
    ...
    def _verify_parameters(
        ...
        if loading_type not in ["basic", "prompt"]:
            raise ValueError(f"{loading_type} loading isn't handled by the TrainerV2.")

        # Learner
        if pipeline_type not in ["basic", "guided"]:
            raise ValueError(f"{pipeline_type} training isn't handled by the TrainerV2.")

        # Model
        if not model_type == "conditioned":
            raise ValueError(f"{model_type} unet isn't handled by the TrainerV2.")
